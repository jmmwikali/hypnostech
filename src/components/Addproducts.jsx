...
const Addproducts = () => {
  ...
  const [product_category, setProductCategory] = useState("Sleep Tracking");
  ...
  const handleSubmit = async (e) => {
    ...
    // Double-check role on submit (belt-and-suspenders)
    if (!isAdmin) {
    ...
    try {
      ...
      setProductName(""); setProductDescription(""); setProductCost(""); setProductCategory("Sleep Tracking"); setProductPhoto("");
      e.target.reset();
  ...
  return (
    <div className='add'>
      ...
      <div className="navbar-wrap">
        ...
        <div>
          <Link className='btn btn-outline-primary me-2' to={"/"} style={{ width: '120px' }}>← Products</Link>
        </div>
      </div>

      <div style={{ padding: '20px 0 0 28px' }}>
        <input type="button" className="back-button" value="← Back" onClick={() => navigate("/")} />
      </div>
      ...
      <div className='row justify-content-center mt-4'>
        <div className="col-md-6 p-4 card shadow">
          ...
          <div style={{ textAlign: 'center', marginBottom: '22px' }}>
            ...
            <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif", color: '#B8C4E8', fontWeight: 400, fontSize: '2rem', margin: '6px 0 0' }}>
              Add a Device
            </h1>
          ...
          <form onSubmit={handleSubmit}>
            <input type="text" placeholder='Product name' className='form-control' required
              value={product_name} onChange={(e) => setProductName(e.target.value)} /><br />
            <input type="text" placeholder='Product description' className='form-control' required
              value={product_description} onChange={(e) => setProductDescription(e.target.value)} /><br />
            <input type="number" placeholder='Price (KES)' className='form-control' required
              value={product_cost} onChange={(e) => setProductCost(e.target.value)} /><br />
            ...
            <select id="product-category" name="product_category" className='form-control'
              value={product_category} onChange={(e) => setProductCategory(e.target.value)}>
            ...
            <input type="submit" value="Add Device" className='button w-100' />
          </form>
        </div>
      </div>
    </div>
